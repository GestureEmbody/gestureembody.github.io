...
Extract the complete ZIP into a folder, then open `index.html`. Keep `assets`, `videos`, `posters`, `stills` and the other included files beside it. Interactive task selection requires JavaScript; a direct-link fallback is included. No external fonts, scripts, analytics, cookies or API keys are required.
...
If externally hosted, choose a neutral URL, avoid linking a personal source repository, check the logged-out visitor view, and follow the applicable submission rules. Upload only this package, not the surrounding workspace or private validation reports. Do not rely exclusively on an external project page for required supplementary material.
